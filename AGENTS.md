...
- **预览库与部署库是两份独立 SQLite**：server 仅在 `users` 表为空时灌种，种子演进
  （如 09-03 新增赵福安爷爷账号）不会自动补灌旧预览库，内容会和部署对不上。
  预览库落后时：停 server → 备份 `data/app.db*` 到 /tmp → 删除后重启即按当前种子重灌
  （2026-09-13 实操：8/31 老库缺赵福安，重灌后与部署基线一致）
