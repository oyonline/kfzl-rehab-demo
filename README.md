# 居家康复智能助手（双端演示原型）

面向脑卒中居家康复场景的 PC 双端可交互原型。演示主线是一条闭环：

> 老人/家属端执行并打卡 → 康复师端远程看到 → 康复师回写指导 → 家属端收到

这条链路回答的是"区别于普通打卡小程序的价值在哪"：康复师不必上门，在工作台就能看到执行情况并给出指导。

## 运行

```bash
pnpm install
pnpm dev
```

**视频文件不在仓库里**（17 个约 390MB，随压缩包另发）。解压到 `public/videos/`，
文件名即视频 id（`v-swallow.mp4` 等）。不解压也能跑，视频页会自动退回分步图文，不会报错。

比赛现场的完整安装与自检步骤见 **[docs/现场部署.md](docs/现场部署.md)** —— 那份是给
实际操作演示电脑的人看的。

| 入口 | 本地地址 | 账号 | 密码 |
|---|---|---|---|
| **角色选择页** | `http://localhost:5173/` | — | — |
| 老人 / 家属端 | `http://localhost:5173/patient` | `chen` | `123456` |
| 康复师端 | `http://localhost:5173/therapist` | `xiaoting` | `123456` |

根路径 `/` 是角色选择页，两个入口各一张卡片。此前 `/` 直接跳 `/patient`，
等于把家属端当成整站门面，康复师端在线上没有入口、只能手输地址。

登录仅账号密码，无验证码、无扫码、无第三方授权。表单已预填，现场无需手动输入
（预填只在开发模式生效，生产构建不含凭据）。

**账号与另一端入口链接刻意不显示在登录页上** —— 那是排练资料，不是产品的一部分，上屏会削弱真实感。需要时查本文件。

**建议开左右两个窗口并排演示**：左边家属端打卡，右边康复师端会自动刷新，无需手动切换。

## 三条硬约束

**1. 两端必须同源部署。**
必须是同一域名下的两个路径（如 `example.com/patient` 与 `example.com/therapist`）。跨域名则 localStorage 不互通，联动闭环直接断裂，只能退回切图演示。

**2. 登录态必须用 sessionStorage，不得改回 localStorage。**
演示数据走 localStorage（同源共享，这是联动的基础）；登录态走 sessionStorage（按标签页隔离）。若登录态放进 localStorage，两端会互相顶掉会话，家属与康复师无法同时在线，并排演示当场不成立。

**3. 不引入任何外部字体、图标库或 CDN。**
图标为内联 SVG，字体用系统字体。现场网络不可控，断网也必须完整呈现。

## 结构

```
src/data/types.ts    双端共用数据契约（已冻结，改动须两端同步）
src/data/seed.ts     虚构案例、入院记录、任务计划、视频清单、打卡历史
src/data/qa.ts       咨询预设答案 ⚠️ 待专业审核
src/data/guidance.ts 饮食与健康指导 ⚠️ 待专业审核
src/data/videoSteps.ts 训练分步说明 ⚠️ 待专业审核
src/store/store.ts   localStorage + 跨标签页订阅
src/auth/auth.ts     账号密码登录（sessionStorage）
src/styles/          设计令牌与组件样式
src/components/      图标、完整档案抽屉、登录守卫
src/pages/patient/   老人 / 家属端：今日、康复咨询、打卡日历、饮食与健康
src/pages/therapist/ 康复师端
```

家属端顶部导航四项：`/patient`（今日）、`/patient/chat`（康复咨询）、`/patient/calendar`（打卡日历）、`/patient/guidance`（饮食与健康）。

内容页统一走**列表 → 详情**：

```
/patient/videos            训练视频列表      → /patient/videos/:id
/patient/guidance          饮食与健康列表    → /patient/guidance/:id
```

训练视频**不占顶部导航**：从今日安排的任务卡点「示范视频」进入 `/patient/videos/:id`，或从今日安排标题旁的「全部训练视频」进入 `/patient/videos`。入口跟着任务走，符合“点开任务节点播放示范视频”的原始要求。

康复师端是**三层结构**，工作台级导航只放工作台级入口，患者内的分页放在详情页里：

```
/therapist                          在管患者列表（工作台首页）
/therapist/inbox                    待处理（跨患者的待回复咨询）
/therapist/patients/:id             患者详情 · 随访概览
/therapist/patients/:id/consult     患者详情 · 咨询记录
/therapist/patients/:id/adherence   患者详情 · 依从性（月日历，与家属端同一组件）
/therapist/patients/:id/guidance    患者详情 · 指导记录
```

列表里 7 位患者都可见，但只有林奶奶有完整档案与实时数据可进入——不为演示编造第二份病例，其余行标注为另一位康复师主责。

## 视频与上传的两个约定

**视频未到位时不做假播放。** `videos[].src` 为空时，播放区显示海报并引导看下方分步图文；填入 src 后 `<video>` 播放器自动接管，图文作为文字说明保留。

**上传是模拟的**（v0.2 §4.2 裁决）：只取文件名与大小这类元数据，**不读取文件内容、不落盘、不上传任何数据**。未选择文件时用一个默认文件名，保证现场即使没备好素材也能演示这一步。

打卡历史用固定模式生成而非随机数，保证每次演示看到的日历完全一致，可反复排练。

## 内容边界

案例人物与数据均为虚构，不对应任何真实个人。

- 不编造用药剂量与评估量表分值 —— 页面一律不展示具体数值，待康复专业人员提供后填入；
- 不得出现"已由医生确认""已真实接入""已同步医院系统"等冒充生产事实的措辞；
- 真实病历原件与直接身份信息不得进入本仓。

## 待补素材与待审内容

### ⚠️ 上台前必须由康复专业人员审核

以下两个文件是本项目起草的医疗性内容，**未经专业审核不得用于正式演示**：

| 文件 | 内容 | 状态 |
|---|---|---|
| `src/data/qa.ts` | 咨询预设答案（呛咳、训练做不动、血压偏高）与自由提问兜底 | REVIEW REQUIRED |
| `src/data/guidance.ts` | 饮食与健康指导四张卡片 | REVIEW REQUIRED |
| `src/data/videoSteps.ts` | 三个训练的分步动作说明 | REVIEW REQUIRED |

起草时遵循的保守原则：不给具体剂量、不改变食物性状比例、不调整训练强度，一律引导"记录 + 观察 + 联系康复师"，每条都带明确的升级条件。

**审核走页面，不再改代码。** 康复师端 → 顶栏「内容审核」（`/therapist/review`），
四类内容分页签逐条通过或驳回：知识库语料 57、咨询预设答案 5、饮食与健康指导 7、
训练分步说明 3。**驳回的内容立即停止对家属展示**（语料退出检索、文案不再下发）。
每次批准与驳回都写审计日志，含操作人与时间，可在同页「查看审核记录」翻阅。

上面这三个文件仍是内容的**初始来源**（由 `pnpm seed` 灌进库），但审核状态只存在
数据库里；改文件头部的 REVIEW 标记不再是审核动作。

### 素材

- 训练示范视频（坐位/下肢、吞咽、翻身转移）尚未到位，到位后填入 `src/data/seed.ts` 的 `videos[].src`，播放器会自动接管；
- 评估量表分值（洼田饮水、MMSE、MMT、Braden）由康复师评估后填入 `assessments[].value`，当前档案页显示为「待录入」；
- 用药剂量同样待专业人员提供，当前只显示药名与服用时间。
